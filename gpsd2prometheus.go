...
var (
	...
	gpsEph = promauto.NewGaugeVec(
		prometheus.GaugeOpts{
			Subsystem: "gpsd",
			Name:      "estimated_error_climb",
			Help:      "Estimated horizontal Position (2D) Error in meters. Also known as Estimated Position Error (epe). Certainty unknown.",
		},
		[]string{
			"device",
		})
...
func main() {
	...
	gps.AddFilter("TPV", func(r interface{}) {
		...
		gpsSpeed.WithLabelValues(tpv.Device).Set(tpv.Speed)
		...
		gpsEph.WithLabelValues(tpv.Device).Set(tpv.Eph)
	})
	...
	gps.AddFilter("DEVICES", func(r interface{}) {
		dev := r.(*gpsd.GSTReport)
		fmt.Println("DEVICES", dev.Path, dev.Flags)
	})

	gps.AddFilter("DEVICES", func(r interface{}) {
		dev := r.(*gpsd.ATTReport)
		fmt.Println("DEVICES", dev.Path, dev.Flags)
	})

	gps.AddFilter("DEVICES", func(r interface{}) {
		dev := r.(*gpsd.VERSIONReport)
		fmt.Println("DEVICES", dev.Path, dev.Flags)
	})

	gps.AddFilter("DEVICES", func(r interface{}) {
		dev := r.(*gpsd.DEVICESReport)
		fmt.Println("DEVICES", dev.Path, dev.Flags)
	})

	gps.AddFilter("DEVICES", func(r interface{}) {
		dev := r.(*gpsd.PPSReport)
		fmt.Println("DEVICES", dev.Path, dev.Flags)
	})

	gps.AddFilter("DEVICES", func(r interface{}) {
		dev := r.(*gpsd.TOFFReport)
		fmt.Println("DEVICES", dev.Path, dev.Flags)
	})

	gps.AddFilter("DEVICES", func(r interface{}) {
		dev := r.(*gpsd.ERRORReport)
		fmt.Println("DEVICES", dev.Path, dev.Flags)
	})
